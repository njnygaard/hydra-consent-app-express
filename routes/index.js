...
router.get('/consent', (r, w) => {
    ...
    refreshToken().then(() => {
        ...
        hydra.getOAuth2ConsentRequest(r.query.consent, resolver((consentRequest) => {
            // consentRequest contains information like requested scopes, client id, ...

            // Here you could, for example, allow clients to force a user's consent. Since you're able to
            // say which scopes a client can request in hydra, you could allow this for a few highly privileged clients!
            //
...
router.get('/', (r,
                 w) => w.send('This is an exemplary consent app for Hydra. Please read the hydra docs on how to use this router.'));
